<form role="form" id="StoreFormEdit" data-action="{{route('stores.update',$data->id)}}" method="post" enctype= multipart/form-data>
    @method('Put')
    @csrf
    <div class="card-body">
        ...
        <div class="form-group">
            ...
            <input type="text" name="storageCapacity" value="{{$data->storageCapacity}}" class="form-control" id="exampleInputEmail1" placeholder="Enter Store Name" data-validation="required">
        </div>
        <div class="form-group">
            ...
            <select class="form-control" name="stores_types_id">
                ...
                @foreach($storesTypes as $storeType)
                    <option value="{{$storeType->id}}" {{$storeType?->id == $data?->stores_types_id ? 'selected' : ''}}>{{$storeType->type}}</option>
                @endforeach
            </select>
        </div>
            <div class="form-group">
...
<script>
    ...
    {
        $('#StoreFormEdit').on('submit',function (e)
        {
